...
// Incluir modelo
$model_path = $_SERVER['DOCUMENT_ROOT'] . '/liceo/modelos/asigna_cargo_modelo.php';
...
$modelo = new AsignaCargoModelo($conn);
...
switch ($action) {
    case 'crear':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($id_profesor) || empty($id_cargo)) {
                $_SESSION['status'] = 'Error: Debe seleccionar profesor y cargo';
                $_SESSION['show_modal'] = true;
            } else {
                if ($modelo->crearAsignacion($id_profesor, $id_cargo)) {
                    $_SESSION['status'] = '✅ Cargo asignado exitosamente al profesor';
                } else {
...
// Incluir vista
$vista_path = $_SERVER['DOCUMENT_ROOT'] . '/liceo/vistas/asigna_cargo_vista.php';
